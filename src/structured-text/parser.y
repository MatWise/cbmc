...
// Structured text language (STL) by Siemens complies with the standards 
...
Zom_Separated_Variable_Name:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_var_entry);
...
Sign_Int_Type_Name:
    ...
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_int);
    }
    ;
...
Sign_DInt_Type_Name:
    ...
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_dint);
    }
    ;
...
Real_Type_Name:
    ...
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_real);
    }
    ;
...
Bool_Type_Name:
	...
	{
	  $$ = $1;
	  parser_stack($$).id(ID_structured_text_bool);
	}
...
FB_Decl:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_function_block);
...
Version_Label:
    ...
    {
      $$ = $3;
      parser_stack($$).type().id(ID_structured_text_version);
    }
    ;
...
Zom_FB_General_Var_Decls:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_var_decls);
    }
    ;
...
Zom_FB_Input_Decl:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_var_input);
    } 
    ;
...
Zom_FB_Output_Decl:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_var_output);
    } 
    ;
...
Func_Decl:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_function);
      parser_stack($$).add_to_operands(std::move(parser_stack($2)),
        std::move(parser_stack($5)));
...
Oom_IL_Network:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_networks);
      parser_stack($$).add_to_operands(std::move(parser_stack($1)));
    }
    ;
...
IL_Network:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_network);
      parser_stack($$).add_to_operands(std::move(parser_stack($4)), 
        std::move(parser_stack($5)));
    }
    ;
...
Opt_Instruction_List:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_instructions);
    }
    ;
...
Opt_Instruction:
    ...
    {
      newstack($$);
      parser_stack($$).id(ID_structured_text_instruction);
    }
    ;
...
IL_Simple_Operator:
    ...
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_load);
    }
    | TOK_TRANSFER
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_transfer);
    }
    | TOK_NOP
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_nop);
    }
    | TOK_CONST_ADD
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_const_add);
    }
    | TOK_ACCU_INT_ADD
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_int_add);
    }
    | TOK_ACCU_INT_SUB
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_int_sub);
    }
    | TOK_ACCU_INT_MUL
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_int_mul);
    }
    | TOK_ACCU_INT_DIV
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_int_div);
    }
    | TOK_ACCU_REAL_ADD
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_real_add);
    }
    | TOK_ACCU_REAL_SUB
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_real_sub);
    }
    | TOK_ACCU_REAL_MUL
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_real_mul);
    }
    | TOK_ACCU_REAL_DIV
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_real_div);
    }
    | TOK_ACCU_DINT_ADD
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_dint_add);
    }
    | TOK_ACCU_DINT_SUB
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_dint_sub);
    }
    | TOK_ACCU_DINT_MUL
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_dint_mul);
    }
    | TOK_ACCU_DINT_DIV
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_accu_dint_div);
    }
    ;
...
IL_Expr_Operator:
    ...
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_and);
    }
    | TOK_AND_NOT
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_and_not);
    } 
    | TOK_OR
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_or);
    } 
    | TOK_OR_NOT
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_or_not);
    }  
    | TOK_XOR 
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_xor);
    }  
    | TOK_XOR_NOT
    {
      $$ = $1;
      parser_stack($$).id(ID_structured_text_xor_not);
    }  
    ;
